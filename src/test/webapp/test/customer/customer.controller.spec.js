...
(function() {
    ...
    var injector = angular.injector(['ng', 'app', 'app.customer', 'app.specs']);
    ...
    QUnit.module('CustomerController Spec', init);

    QUnit.test('"CustomerController" initialised successfully', function(assert) {
    ...
    QUnit.test('"CustomerController" correctly attempts to create a Contact.', function(assert) {
        expect(1);
        delete this.testData[0].id;
        this.$httpBackend.expectPOST('rest/customers', this.testData[0]).respond(409, 'Conflict');
        $controller('CustomerController', {
            $scope: this.$scope,
            $routeParams: {},
            $location: $location,
    ...
    QUnit.test('"CustomerController" correctly handles routeParams and requests the appropriate Contact.', function(assert) {
    ...
    QUnit.test('"CustomerController" correctly attempts to update a specified Contact.', function(assert) {
        expect(1);
        this.$httpBackend.expectGET(/rest\/customers\/10001[?&]_=[0-9]+$/).respond(200, this.testData[1]);
        $controller('CustomerController', {
            $scope: this.$scope,
            $routeParams: {contactId: 10001},
            $location: $location,
        ...
        this.$httpBackend.expectPUT('rest/customers/10001', this.testData[1]).respond(200);
        this.$scope.saveContact();
        this.$httpBackend.flush();
        assert.equal(this.$scope.contacts.data[1].firstName, 'Jack');
    });

    QUnit.test('"CustomerController" correctly attempts to delete a specified Contact.', function(assert) {
        expect(1);
        this.$httpBackend.expectGET(/rest\/customers\/10001[?&]_=[0-9]+$/).respond(200, this.testData[1]);
        $controller('CustomerController', {
            $scope: this.$scope,
        ...
        this.$httpBackend.flush();
        this.$httpBackend.resetExpectations();
        this.$httpBackend.expectDELETE('rest/customers/10001').respond(204);
